...
export class MailService {
  ...
  async sendWelcomeEmail(payload: WelcomeEmailPayload): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Echec envoi email a ${email} : ${(error as Error).message}`,
      );
      throw error;
    }
  }

  private buildWelcomeHtml(data: WelcomeTemplateData): string {
    return `
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body style="margin:0;padding:0;background-color:#f4f4f4;font-family:Arial,sans-serif;">
  ...
  private buildWelcomeText(data: WelcomeTemplateData): string {
    return [
